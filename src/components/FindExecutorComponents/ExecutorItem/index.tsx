...
const ExecutorItem: FC<{
  ...
  useEffect(() => {
    setReservedService(
      executor.services.find((service) => service.id === findExecutorState.servicesTypesId[0])
    );
  }, [executor.services, findExecutorState.servicesTypesId]);

  return (
    <div
      key={executor.id}
      className={`${styles.executor} ${
        additionalStyles ? additionalStyles.executorForCarousel : ''
      }`}
    >
      <div className={styles.executorInfo}>
        ...
        <div className={styles.executorTextBlock}>
          ...
          {executor.text && (
            <p
              className={`${styles.executorText} ${
                additionalStyles ? additionalStyles.executorTextForCarousel : ''
              }`}
            >
              {executor.isTextHidden &&
              (showAllExecutorText || executor.text.split(' ').length > 10)
                ? getShorterText(executor.text)
                : executor.text}
              {executor.isTextHidden && executor.text.split(' ').length > 10 && (
                <img
                  onClick={() => showAllExecutorText && showAllExecutorText(executor.id)}
      ...
      {!!executor.services.length && (
        <div
          className={`${styles.services} ${
            additionalStyles ? additionalStyles.servicesForCarousel : ''
          }`}
        >
          {reservedService && (
            <div
              className={`${styles.service} ${
                additionalStyles ? additionalStyles.serviceForCarousel : ''
              }`}
            >
              <p className={styles.serviceName}>
                {reservedService.group_name} {reservedService.name}
              </p>
              <p className={styles.servicePrice}>
                от {modifyPrice(reservedService.price)} за ед.
          ...
          {executor.services
            ...
            .filter((service) => {
              ...
              return service.id !== reservedService?.id;
            })
            .slice(
              0,
              servicesNumber
          ...
          {executor.areServicesHidden && showAllExecutorServices && (
            <div onClick={() => showAllExecutorServices(executor.id)} className={styles.service}>
      ...
      <div
        className={`${styles.executorButtons} ${
          additionalStyles ? additionalStyles.executorButtonsForCarousel : ''
        }`}
      >
        <button
        ...
        <button
          ...
          className={`${styles.executorOfferButton} ${
            additionalStyles ? additionalStyles.executorOfferButtonCarousel : ''
          }`}
        >
          Предложить тендер
          <img
            src={
              additionalStyles
                ? '/find-executor/arrow-right-black.svg'
                : '/find-executor/arrow-right-white.svg'
            }
