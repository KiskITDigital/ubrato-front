  ...
  useEffect(() => {
    if (location.pathname === "/find-executor") {
      client
        ...
        .then(async (response) => {
          if (response.hits) setQuerySuggestions([...response.hits]);
        })
        .catch((error) => {
    ...
    <div className={`container ${styles.container}`}>
      ...
      <div className="w-full relative">
        <label className={styles.inputFilterLabel}>
          ...
          <input
            ...
            placeholder={
              location.pathname === "/alltenders"
                ? "Например, название, описание, услуга"
                : "Например, наименование или ИНН компании"
            }
          />
        </label>
      ...
      {breadCrumbs.length > 1 && (
        <div className={styles.breadCrumbs}>
          {breadCrumbs.map(
            (
              crumb: { name: string; onClick?: () => void },
              ind: number,
              breadCrumbsArr: { name: string; onClick?: () => void }[]
            ) => (
              <Fragment key={ind}>
                <p
                  className={`${styles.breadCrumb} ${
                    crumb?.onClick ? "" : styles.breadCrumbInActive
                  }`}
                ...
                {ind < breadCrumbsArr.length - 1 && (
                  <img
                    className={`${styles.breadCrumbImg} ${
                      crumb.onClick ? "" : styles.breadCrumbImgInActive
                    }`}
