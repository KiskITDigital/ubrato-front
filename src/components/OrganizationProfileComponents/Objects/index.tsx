...
const Objects: FC<{ objects: ObjectV[] }> = ({ objects }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const newObjects = await generateTypesenseClient("object_type_index", {
        filter_by: `id:[${servicesFilters}]`,
        include_fields: "$object_group_index(id, name)",
        per_page: 250,
      });
      if (newObjects)
        setModifiedObjects(
          Object.entries(
            groupBy(
              newObjects.hits?.map((el) => el.document) as ModifiedObject[],
              (object: ModifiedObject) => object.object_group_index.name
            )
          )
        );
    })();
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.objects}>
        {modifiedObjects.map((objectGroup, ind) => (
          <div key={ind} className={styles.objectGroup}>
            <p className={styles.objectGroupName}>{objectGroup[0]}</p>
            <div className={styles.objectTypes}>
              {objectGroup[1].map((objectType) => (
                <p key={objectType.id} className={styles.objectType}>
                  {objectType.name}
                </p>
              ))}
            </div>
