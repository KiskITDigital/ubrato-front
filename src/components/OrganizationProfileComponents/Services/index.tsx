...
const Services: FC<{ services: Service[] }> = ({ services }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const newServices = await generateTypesenseClient('service_type_index', {
            ...
            groupBy(
              newServices.hits
                ...
                .map((service) => {
                  ...
                  if ('id' in service)
                    serviceToFindPrice =
                      services.find((el) => el.id.toString() === service.id)?.price || 0;
                  return { ...service, price: serviceToFindPrice };
                }) as ModifiedService[],
              (service: ModifiedService) => service.service_group_index.name
            )
          )
        );
    })();
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.services}>
        {modifiedServices.map((serviceGroup, ind) => (
          <div key={ind} className={styles.serviceGroup}>
            <p className={styles.serviceGroupName}>{serviceGroup[0]}</p>
            <div className={styles.serviceTypes}>
              {serviceGroup[1].map((serviceType) => (
                <div className={styles.serviceType} key={serviceType.id}>
