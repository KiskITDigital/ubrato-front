        ...
        <>
          <div className="flex gap-4 items-start mt-5">
            ...
            <p className="text-[16px] text-[#666] max-w-[700px]">
              Если необходимо опубликовать дополнительную, уточняющую информацию о тендере,
              разместите ее здесь и отправьте на модерацию
            </p>
          </div>
            ...
            <label htmlFor="infoFile">
              ...
              {newLinks.length < 4 && (
                <div className="flex py-[10px] w-full justify-center items-center gap-10 border-accent border-dashed border rounded-[10px] cursor-pointer">
                  <img className="w-8 h-8" src="/add-file-ic.svg" alt="addFile" />
                  <div>
            ...
            <div className="flex gap-3">
              {newLinks.map((link, ix) => (
                <div key={ix} className="relative self-start">
                  ...
                  >
                    <img className="w-[12px] h-[12px]" src="/x-icon.svg" alt="delete" />
                  </button>
                  <img className="h-[100px] w-auto" src={`https://cdn.ubrato.ru/s3${link}`} />
                </div>
              ))}
            </div>
