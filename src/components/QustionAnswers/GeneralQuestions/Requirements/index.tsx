...
const Requirements: FC = () => {
    ...
    return (
        <div className={qstyles.container}>
            ...
            {
                <Accordion variant="splitted" itemClasses={itemClasses} className={styles.accordionWrapper}
                ...
                >
                    {
                        categories.map(category => (
                            <AccordionItem indicator={<ExpandButton isActive={category.id.toString() === questionNumber.toString()} />} key={category.id} aria-label={category.title} title={category.title}>
                                {category.component}
                            </AccordionItem>
                        ))
                    }
