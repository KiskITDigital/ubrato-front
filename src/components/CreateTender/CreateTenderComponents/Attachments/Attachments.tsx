...
const Attachments: FC<{ windowWidth: number; ref?: React.LegacyRef<HTMLDivElement> }> = forwardRef<
  HTMLDivElement,
          ...
          {!!createTenderState.attachments.length && (
            <div className={`${styles.section__attachments__block__cardList}`}>
              {createTenderState.attachments.map((img, ind) => (
                <div key={img.id} className={`${styles.section__attachments__block__cardItem}`}>
                  {img.fileType === 'image' && windowWidth > 1050 ? (
                    <img
                      className={`${styles.section__attachments__block__cardItem__img}`}
                      src={
                        ind < createTenderState.attachments.length
                          ? img.linkToSend
                    ...
                    >
                      <img
                        className={styles.section__attachments__block__cardItem__notImageInfo__img}
                        src={
                          img.fileType === 'image'
                            ? '/create-tender/create-tender-img-mobile.svg'
                            : '/create-tender/create-tender-file.svg'
                        }
                        alt=""
                      />
                      <div className={styles.section__attachments__block__cardItem__notImageInfo}>
                        ...
                        {windowWidth <= 1050 && (
                          <img
                            ...
                            src="/create-tender/create-tender-close-gray.svg"
                            alt=""
                          />
                        )}
                      </div>
                    ...
                    <>
                      ...
                      <div className={`${styles.section__attachments__block__cardItem__changes}`}>
                        <img
                          ...
                          src="/create-tender/create-tender-change-attachment.svg"
                          alt=""
                          onClick={() => {
                        ...
                        <input
                          ...
                          type="file"
                          multiple
                          accept=".doc,.docx,.xls,.xlsx,.jpg,.jpeg,.pdf"
                          onChange={(e) => {
                            createTenderState.handleFileUpload(
                              e,
                              createTenderState.attachmentIdToChange
                            );
          ...
          <div className="flex gap-5">
            ...
            <div className="flex gap-[10px] bg-[--color-light-gray-secondary] p-3 rounded-[10px]">
              ...
              <p className="leading-tight text-[16px]">
                Загрузите файлы с информацией об объекте - фото, план или другие материалы. До 8
                файлов в форматах doc, docx, xls, xlsx, jpg или pdf, не более 10 Мб каждый.
              </p>
            </div>
          </div>
          <input
            ...
            onChange={(e) => {
              if (e.target.files) {
                ...
                else createTenderState.handleFileUpload(e, null);
              }
            }}
            ref={inputFileRef}
            style={{ display: 'none' }}
