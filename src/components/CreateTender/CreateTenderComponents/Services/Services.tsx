...
const Services: FC<{ windowWidth: number; ref?: React.LegacyRef<HTMLDivElement> }> = forwardRef<
  HTMLDivElement,
      ...
      {windowWidth <= 1050 ? (
        <>
          ...
          <button
            ...
            className={`${styles.section__block__button} ${styles.textRegular} ${
              createTenderState.errors.includes("object") ? styles.section__block__buttonError : ""
            }`}
          >
            <img
              src={
          ...
          {isChoosingNewServiceNameMobile && (
            ...
            >
              {!!cleaningTypeStore?.apiCleaningTypes?.length &&
              ...
              {createTenderState.services.map((service) => (
                <div className={styles.services__block__serviceToChange} key={service.id}>
                  <div key={service.id} className={`${styles.services__block__service}`}>
                    ...
                    <img
                      className={`${styles.service__name__img} ${styles.arrowRightImg}`}
                      src="/create-tender/create-tender-arrow-right.svg"
                      alt=""
                    />
                    <div className={`${styles.services__block__service__types}`}>
                      {service.types.map((type) => (
                        ...
                        >
                          ...
                          ></span>
                          <img
                            onClick={() => createTenderState.removeServiceType(service.id, type.id)}
                            className={`${styles.section__block__add__object__objectCategory__img}`}
                            src="/create-tender/create-tender-close-white.svg"
                            alt=""
                          />
                        </p>
                      ))}
                    ...
                    <button
                      ...
                      className={`${styles.section__block__button} ${styles.service__button} ${styles.services__block__service__change} ${styles.textRegular}`}
                    >
                      Изменить
                    </button>
                    <button
                    ...
                    >
                      <img src="/create-tender/create-tender-close.svg" alt="" />
                    </button>
                  </div>
                  {isChoosingServiceToChange === service.id && (
                    <div className={`${styles.object__types} ${styles.services__types}`}>
                      <div className={styles.object__services__mobile}>
                                ...
                                ?.types.map((type) => (
                                  <Checkbox
                                    className={`${styles.object__objects__types__p} ${
                                      ...
                                      chooseTypesTypesToObjectToChangeService.includes(type.name)
                                        ? `${styles.CheckboxNextUIActive} ${styles.CheckboxNextUIActiveTypes}`
                                        : ""
                                    }`}
                                    key={type.id}
                  ...
                  <div className={`${styles.scroll} h-[452px] !w-full`}>
                    {chooseTypesNameToObjectToAddService && (
                      <>
                        ...
                        >
                          {cleaningTypeStore.apiCleaningTypes
                            .find((service) => service.name === chooseTypesNameToObjectToAddService)
                            ?.types.map((type) => (
                              <Checkbox
                                className={`${styles.object__objects__types__p} ${
                                  styles.CheckboxNextUI
