...
const CleaningTZ: FC<{ ref?: React.LegacyRef<HTMLDivElement> }> = forwardRef<
  HTMLDivElement,
      ...
      <div className={`${styles.section__block}`}>
        ...
        {createTenderState.cleaningTZ && (
          <p className={`${styles.textMedium} ${styles.cleaningTZText}`}>
            {createTenderState.cleaningTZ.fileName}
            <span className={styles.textGray}>
              , {formatFileSize(createTenderState.cleaningTZ.fileSize)}
            </span>
          </p>
        )}
        <button
          ...
          className={`${styles.textRegular} ${
            createTenderState.errors.includes('tz') ? cleaningTZStyles.cleaningTZError : ''
          }`}
        >
          {createTenderState.cleaningTZ ? (
            <img
              className={styles.removeTZ}
            ...
            <div className="flex gap-5 cursor-default">
              <div
                className="flex items-center gap-2 bg-[--color-light-gray-secondary] px-3 h-[34px] rounded-[10px] cursor-pointer"
              ...
              <div className="flex gap-[10px] bg-[--color-light-gray-secondary] p-3 rounded-[14px] text-start text-[16px]">
                ...
                <p className=" leading-tight cursor-text select-text">
                  Загрузите файл с техническим заданием на уборку объекта в форматах doc, docx, xls,
                  xlsx, jpg или pdf. Допустимый размер файла — 10 Мб.
                </p>
              </div>
            </div>
          )}
        </button>
        {createTenderState.errors.includes('tz') && (
          <p className={`${styles.inputErrorText} ${cleaningTZStyles.cleaningTZErrorText}`}>
        ...
        <input
          ...
          onChange={(e) => {
            if (e.target.files) {
              if (e.target.files[0].size > 10485760) alert('Недопустимый формат или размер файла.');
              else createTenderState.handleFileUpload(e, null, 'upload-tz');
            }
          }}
          ref={inputFileRef}
          style={{ display: 'none' }}
