...
const OrganizationProfilePage: FC = () => {
  ...
  const [data, setData] = useState<ErrorInfo | ExecutorProfileInfo | OrdererProfileInfo | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (org_id) {
      (async () => {
        ...
        const token = localStorage.getItem('token');
        const isFavorite =
          (!!token && (await isFavoriteExecutor(org_id, token))?.data?.status) || false;
        setData({
          ...fetchedData,
          isFavorite,
        });
