...
export const ForgotPassword: FC = () => {
        ...
        try {
          ...
          if (error instanceof AxiosError) {
            ...
            if (error.response?.status === 404) {
              formik.setErrors({
                email:
                  "Адрес электронной почты не найден. Проверьте правильность введенных данных.",
              });
            }
          }
        }
  ...
  const inputClasses = {
    ...
    helperWrapper: "block self-end absolute bottom-[0px]",
    inputWrapper: "p-0",
  };
  ...
  return (
    <div className="container flex items-center justify-center h-[calc(100vh-110px)]">
      <div className="w-[478px] flex flex-col gap-11">
        ...
        <p className="text-lg">
          Введите адрес электронной почты, чтобы подтвердить действие и сбросить
          пароль. Вы будете перенаправлены на страницу для ввода нового пароля
        </p>
          ...
          <div className="fixed top-0 left-0 w-[100vw] h-[100vh] z-10 backdrop-blur-[20px] bg-[rgba(0,0,0,.2)] flex justify-center items-center">
            <div className="bg-white rounded-[20px] p-5 flex flex-col gap-9">
              ...
              <div className="flex gap-[10px] py-3 px-[14px] bg-[rgba(0,0,0,.03)] rounded-[14px]">
                <img
                  src="info-blue-ic.svg"
                  alt="icon"
                  className="w-[16px] h-[16px]"
                />
                <p className="w-[523px] font-medium text-lg leading-5">
                  На указанную вами почту было отправлено письмо с ссылкой для
                  восстановления пароля, перейдите по ней для подтверждения
                  действия
                </p>
              </div>
              <button
